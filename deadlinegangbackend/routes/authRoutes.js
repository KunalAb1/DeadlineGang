...
router.get('/', (req, res) => {
    res.json({ message: 'Auth route home' })
})
...
router.post('/sendotp', async (req, res, next) => {
    ...
    try {
        ...
        const newVerification = new Verification({ email, code });
...
router.post('/login', async (req, res, next) => {
    try {
        ...
        if (!user) return responseFunction(res, 400, 'Invalid credentials', null, false);

        const isMatch = await bcrypt.compare(password, user.password);
        if (!isMatch) return responseFunction(res, 400, 'Invalid credentials', null, false);
...
router.get('/checklogin', authTokenHandler, async (req, res, next) => {
    ...
    res.json({ ok: req.ok, message: req.message, userId: req.userId });
});
